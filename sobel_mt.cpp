...
void *runSobelMT(void *ptr)
{
  ...
  while (1) {
    ...
    pc_start(&perf_counters);
    if (myID == thread0_id) {
      // Mat img_gray_top = img_gray.rowRange(0, img_gray.rows / 2 + 2);
      Mat img_sobel_top = img_sobel.rowRange(0, img_sobel.rows / 2);
      sobelCalc(img_gray, img_sobel_top);
    } else {
      // Mat img_gray_bot = img_gray.rowRange(img_gray.rows / 2 - 2, img_gray.rows);
      Mat img_sobel_bot = img_sobel.rowRange(img_sobel.rows / 2, img_sobel.rows);
      sobelCalc(img_gray, img_sobel_bot);
    }
    pc_stop(&perf_counters);
